    ...
    $.fn.InitUploader = function (b) {
        var fun = function (parentObj) {
            ...
            var uploader = WebUploader.create({
                ...
                fileSingleSizeLimit: p.filesize * 10240 //文件大小
            });

            $(".webuploader-element-invisible").parent().css("top","0px");
            ...
            uploader.on('uploadSuccess', function (file, data) {
                ...
                if (data.status == '1') {
                    //如果是单文件上传，则赋值相应的表单
                    if (!p.multiple) {
                        parentObj.parent().find(".imgview").attr("src",data.path);
                        parentObj.parent().find(".upload-path").val(data.path);
